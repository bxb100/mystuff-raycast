      ...
      {devices.map((device) => (
        <List.Item
          ...
          accessories={[
            ...
            {
              text: device.battery.time_to_empty_min
                ? `Empy after ${device.battery.time_to_empty_min} min`
                : null,
            },
          ]}
          actions={
            <ActionPanel>
              ...
              {device.capabilities.includes("CAP_NOTIFICATION_SOUND") ? (
                <Action
                  ...
                  icon={Icon.Bell}
                  onAction={() => playNotification(device)}
                />
              ) : null}
              {device.capabilities.includes("CAP_LIGHTS") ? (
                <>
                  <Action
                    ...
                    onAction={() => toggleLights(device, 0)}
                  />
                  <Action
                    title={"Enable Lights"}
                    icon={Icon.LightBulb}
                    onAction={() => toggleLights(device, 1)}
                  />
                </>
              ) : null}
              {device.capabilities.includes("CAP_SIDETONE") ? (
                <>
                  <Action
                    ...
                    onAction={() => sidetoneSet(device, 128)}
                  />
                  <Action
